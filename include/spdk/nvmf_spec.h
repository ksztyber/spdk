...
enum spdk_nvmf_auth_type {
	...
	SPDK_NVMF_AUTH_TYPE_DH_HMAC_CHAP	= 0x1,
};

/** AUTH message identifiers */
enum spdk_nvmf_auth_id {
	SPDK_NVMF_AUTH_ID_NEGOTIATE	= 0x00,
	SPDK_NVMF_AUTH_ID_CHALLENGE	= 0x01,
	SPDK_NVMF_AUTH_ID_REPLY		= 0x02,
	SPDK_NVMF_AUTH_ID_SUCCESS1	= 0x03,
	SPDK_NVMF_AUTH_ID_SUCCESS2	= 0x04,
	SPDK_NVMF_AUTH_ID_FAILURE2	= 0xf0,
	SPDK_NVMF_AUTH_ID_FAILURE1	= 0xf1,
};

/** Hash function identifiers */
enum spdk_nvmf_auth_hash {
	SPDK_NVMF_AUTH_HASH_NONE	= 0x0,
	SPDK_NVMF_AUTH_HASH_SHA256	= 0x1,
	SPDK_NVMF_AUTH_HASH_SHA384	= 0x2,
	SPDK_NVMF_AUTH_HASH_SHA512	= 0x3,
};

/** Diffie-Hellman group identifiers */
enum spdk_nvmf_auth_dhgroup {
	SPDK_NVMF_AUTH_DHGROUP_NULL = 0x0,
	SPDK_NVMF_AUTH_DHGROUP_2048 = 0x1,
	SPDK_NVMF_AUTH_DHGROUP_3072 = 0x2,
	SPDK_NVMF_AUTH_DHGROUP_4096 = 0x3,
	SPDK_NVMF_AUTH_DHGROUP_6144 = 0x4,
	SPDK_NVMF_AUTH_DHGROUP_8192 = 0x5,
};
...
#define SPDK_NVMF_AUTH_SC_NO_CONCAT	0
/** Secure channel concatenation with TLS */
#define SPDK_NVMF_AUTH_SC_CONCAT	1
...
struct spdk_nvmf_auth_challenge {
	uint8_t		auth_type;
	uint8_t		auth_id;
	uint8_t		reserved0[2];
	uint16_t	t_id;
...
SPDK_STATIC_ASSERT(sizeof(struct spdk_nvmf_auth_challenge) == 16, "Incorrect size");

struct spdk_nvmf_auth_reply {
	uint8_t		auth_type;
	uint8_t		auth_id;
	uint8_t		reserved0[2];
	uint16_t	t_id;
	...
	uint32_t	seqnum;
	uint8_t		rval[0];
	/* Followed by cval if cvalid == 1 and dval if dhvlen > 0 */
};
SPDK_STATIC_ASSERT(sizeof(struct spdk_nvmf_auth_reply) == 16, "Incorrect size");

struct spdk_nvmf_auth_success1 {
	uint8_t		auth_type;
	uint8_t		auth_id;
	uint8_t		reserved0[2];
	uint16_t	t_id;
...
SPDK_STATIC_ASSERT(sizeof(struct spdk_nvmf_auth_success1) == 16, "Incorrect size");

struct spdk_nvmf_auth_success2 {
	uint8_t		auth_type;
	uint8_t		auth_id;
	uint8_t		reserved0[2];
	uint16_t	t_id;
...
SPDK_STATIC_ASSERT(sizeof(struct spdk_nvmf_auth_success2) == 16, "Incorrect size");
...
enum spdk_nvmf_auth_failure_reason {
	...
	SPDK_NVMF_AUTH_SC_CONCAT_MISMATCH		= 0x3,
