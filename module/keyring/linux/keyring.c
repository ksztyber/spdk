...
linux_probe_key(const char *name)
{
	...
	return spdk_keyring_add(&opts);
}

static int
linux_add_key(struct spdk_key *key, const struct spdk_json_val *params)
{
...
static void
linux_dump_key(struct spdk_key *key, struct spdk_json_write_ctx *w)
{
	struct linux_key *lkey = spdk_key_get_ctx(key);
