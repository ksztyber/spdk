...
static struct spdk_bdev_nvme_opts g_opts = {
	...
	.chap_digests = SPDK_BIT(SPDK_NVMF_AUTH_HASH_SHA256) |
			SPDK_BIT(SPDK_NVMF_AUTH_HASH_SHA384) |
			SPDK_BIT(SPDK_NVMF_AUTH_HASH_SHA512),
	.chap_dhgroups = SPDK_BIT(SPDK_NVMF_AUTH_DHGROUP_NULL) |
...
_nvme_ctrlr_delete(struct nvme_ctrlr *nvme_ctrlr)
{
	...
	spdk_keyring_put(nvme_ctrlr->chap_key);
	free(nvme_ctrlr);
...
free_nvme_async_probe_ctx(struct nvme_async_probe_ctx *ctx)
{
	...
	spdk_keyring_put(ctx->drv_opts.chap_key);
	free(ctx);
}
		  ...
		  struct nvme_async_probe_ctx *ctx)
{
	...
	if (ctx != NULL) {
		if (ctx->drv_opts.tls_psk != NULL) {
			nvme_ctrlr->psk = spdk_keyring_get(spdk_key_get_name(ctx->drv_opts.tls_psk));
		...
		if (ctx->drv_opts.chap_key != NULL) {
			nvme_ctrlr->chap_key = spdk_keyring_get(
		 ...
		 bool multipath)
{
	...
	if (ctx->bdev_opts.psk[0] != '\0') {
		...
		ctx->drv_opts.tls_psk = spdk_keyring_get(ctx->bdev_opts.psk);
		if (ctx->drv_opts.tls_psk == NULL) {
	...
	if (ctx->bdev_opts.chap_key != NULL) {
		ctx->drv_opts.chap_key = spdk_keyring_get(ctx->bdev_opts.chap_key);
		if (ctx->drv_opts.chap_key == NULL) {
