...
rpc_decode_digest(const struct spdk_json_val *val, void *out)
{
	...
	rc = spdk_nvme_auth_get_digest_id(digest);
	if (rc >= 0) {
		*flags |= SPDK_BIT(rc);
		rc = 0;
	}
...
rpc_decode_digest_array(const struct spdk_json_val *val, void *out)
{
	...
	*flags = 0;
	...
	return spdk_json_decode_array(val, rpc_decode_digest, out, 0, &count, 0);
}
...
rpc_decode_dhgroup(const struct spdk_json_val *val, void *out)
{
	...
	rc = spdk_nvme_auth_get_dhgroup_id(dhgroup);
	if (rc >= 0) {
		*flags |= SPDK_BIT(rc);
		rc = 0;
	}
...
rpc_decode_dhgroup_array(const struct spdk_json_val *val, void *out)
{
	...
	*flags = 0;
	...
	return spdk_json_decode_array(val, rpc_decode_dhgroup, out, 0, &count, 0);
}
...
static const struct spdk_json_object_decoder rpc_bdev_nvme_options_decoders[] = {
	...
	{"chap_digests", offsetof(struct spdk_bdev_nvme_opts, chap_digests), rpc_decode_digest_array, true},
	{"chap_dhgroups", offsetof(struct spdk_bdev_nvme_opts, chap_dhgroups), rpc_decode_dhgroup_array, true},
};
...
static const struct spdk_json_object_decoder rpc_bdev_nvme_attach_controller_decoders[] = {
	...
	{"chap_key", offsetof(struct rpc_bdev_nvme_attach_controller, chap_key), spdk_json_decode_string, true},
	{"chap_ctrlr_key", offsetof(struct rpc_bdev_nvme_attach_controller, chap_ctrlr_key), spdk_json_decode_string, true},
};
				...
				const struct spdk_json_val *params)
{
	...
	ctx->req.bdev_opts.chap_key = ctx->req.chap_key;
	ctx->req.bdev_opts.chap_ctrlr_key = ctx->req.chap_ctrlr_key;
