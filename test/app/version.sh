...
get_header_version() {
	grep -E "^#define SPDK_VERSION_${1^^}[[:space:]]+" "$rootdir/include/spdk/version.h" | cut -f2
}

major=$(get_header_version major)
minor=$(get_header_version minor)
patch=$(get_header_version patch)
suffix=$(get_header_version suffix | tr -d \")
...
PYTHONPATH="$PYTHONPATH:$rootdir/python" py_version=$(python -c 'import spdk; print(spdk.__version__)')
[[ "$py_version" == "$version" ]]
