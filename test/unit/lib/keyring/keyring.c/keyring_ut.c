...
static int
ut_keyring_add_key(struct spdk_key *key, const struct spdk_json_val *opts)
{
	...
	memcpy(utkey->buf, opts->start, opts->len);
	utkey->len = opts->len;
...
test_keyring_add_remove(void)
{
	...
	spdk_keyring_remove("key0");
	CU_ASSERT_PTR_NULL(spdk_keyring_get("key0"));
	...
	/* Try to add a key with a non-existing module */
	opts.size = SPDK_SIZEOF(&opts, opts);
	opts.name = "key0";
	opts.module = "non-existing";
	opts.opts = &params;
	rc = spdk_keyring_add(&opts);
