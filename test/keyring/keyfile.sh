...
bperf_cmd keyring_add_key key0 keyfile --keyfile-path "$key0path"
bperf_cmd keyring_add_key key1 keyfile --keyfile-path "$key1path"
...
# Remove both keys
bperf_cmd keyring_remove_key key0
bperf_cmd keyring_remove_key key1
(($(bperf_cmd keyring_get_keys | jq "length") == 0))

# Try to add a key that has too relaxed permissions
chmod 0660 "$key0path"
NOT bperf_cmd keyring_add_key key0 keyfile --keyfile-path "$key0path"

# Remove the keyfile after adding it to the keyring
chmod 0600 "$key0path"
bperf_cmd keyring_add_key key0 keyfile --keyfile-path "$key0path"
rm -f "$key0path"
