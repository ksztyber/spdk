...
parse_args(int argc, char **argv, struct spdk_env_opts *env_opts)
{
	...
	while ((op = getopt_long(argc, argv, PERF_GETOPT_SHORT, g_perf_cmdline_opts, &long_idx)) != -1) {
		...
		case PERF_NUMBER_IOS:
			rc = spdk_parse_capacity(optarg, &valu64, NULL);
			if (rc != 0) {
				fprintf(stderr, "Converting a string to integer failed\n");
				return 1;
			}
			switch (op) {
			case PERF_IO_SIZE:
				g_io_size_bytes = (uint32_t)valu64;
				break;
			case PERF_IO_UNIT_SIZE:
				g_io_unit_size = (uint32_t)valu64;
				break;
			case PERF_ZEROCOPY_THRESHOLD:
				g_sock_zcopy_threshold = (uint32_t)valu64;
				break;
			case PERF_BUFFER_ALIGNMENT:
				g_io_align = (uint32_t)valu64;
