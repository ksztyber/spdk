...
spdk_bdev_set_opts(struct spdk_bdev_opts *opts)
{
	...
	if (opts->small_buf_pool_size != BUF_SMALL_POOL_SIZE) {
		SPDK_ERRLOG("spdk_bdev_opts.small_buf_pool_size is deprecated, use spdk_iobuf_set_opts() instead\n");
	}

	if (opts->large_buf_pool_size != BUF_LARGE_POOL_SIZE) {
		SPDK_ERRLOG("spdk_bdev_opts.large_buf_pool_size is deprecated, use spdk_iobuf_set_opts() instead\n");
	}
