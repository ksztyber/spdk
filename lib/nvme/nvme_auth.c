...
static const struct nvme_auth_digest g_digests[] = {
	...
	{ SPDK_NVMF_AUTH_HASH_SHA384, "sha384", 48 },
	{ SPDK_NVMF_AUTH_HASH_SHA512, "sha512", 64 },
};

static const struct nvme_auth_dhgroup g_dhgroups[] = {
	{ SPDK_NVMF_AUTH_DHGROUP_NULL, "null" },
	{ SPDK_NVMF_AUTH_DHGROUP_2048, "ffdhe2048" },
	{ SPDK_NVMF_AUTH_DHGROUP_3072, "ffdhe3072" },
	{ SPDK_NVMF_AUTH_DHGROUP_4096, "ffdhe4096" },
	{ SPDK_NVMF_AUTH_DHGROUP_6144, "ffdhe6144" },
	{ SPDK_NVMF_AUTH_DHGROUP_8192, "ffdhe8192" },
};
...
const char *
spdk_nvme_auth_get_dhgroup_name(int id)
{
	size_t i;
...
nvme_auth_digest_allowed(struct spdk_nvme_qpair *qpair, uint8_t digest)
{
	...
	return ctrlr->opts.chap_digests & (1 << digest);
}
...
nvme_auth_dhgroup_allowed(struct spdk_nvme_qpair *qpair, uint8_t dhgroup)
{
	...
	return ctrlr->opts.chap_dhgroups & (1 << dhgroup);
}
			...
			const void *keyin, size_t keylen, void *out, size_t outlen)
{
	...
	params[0] = OSSL_PARAM_construct_utf8_string("digest",
			(char *)spdk_nvme_auth_get_digest_name(hash), 0);
			    ...
			    void *caval, size_t *calen, enum spdk_nvmf_auth_hash hash)
{
	...
	md = EVP_MD_fetch(NULL, spdk_nvme_auth_get_digest_name(hash), NULL);
	if (!md) {
...
nvme_auth_calc_response(struct spdk_key *key, enum spdk_nvmf_auth_hash hash,
			...
			const void *cval, size_t clen, void *rval, size_t rlen)
{
	EVP_MAC *hmac;
	EVP_MAC_CTX *ctx;
	OSSL_PARAM params[2];
	...
	digest = spdk_nvme_auth_get_digest_name(hash);
	...
	if (EVP_MAC_final(ctx, rval, &hmaclen, rlen) != 1) {
		goto out;
	}
...
nvme_auth_generate_dhkey(void *pub, size_t *len, enum spdk_nvmf_auth_dhgroup dhgroup)
{
	...
	if (ctx == NULL) {
		SPDK_ERRLOG("Failed to create dhkey ctx\n");
		goto error;
	}

	if (EVP_PKEY_keygen_init(ctx) != 1) {
		SPDK_ERRLOG("Failed to initialize dhkey keygen\n");
		goto error;
	}

	params[0] = OSSL_PARAM_construct_utf8_string("group",
			(char *)spdk_nvme_auth_get_dhgroup_name(dhgroup), 0);
	params[1] = OSSL_PARAM_construct_end();
	if (EVP_PKEY_CTX_set_params(ctx, params) != 1) {
		SPDK_ERRLOG("Failed to initialize dhkey ctx\n");
		goto error;
	}
	if (EVP_PKEY_generate(ctx, &key) != 1) {
		SPDK_ERRLOG("Failed to generate dhkey\n");
		goto error;
	}
	if (EVP_PKEY_get_bn_param(key, "pub", &bn) != 1) {
		SPDK_ERRLOG("Failed to get dhkey pubkey\n");
		goto error;
	}

	if ((size_t)BN_num_bytes(bn) > *len) {
		SPDK_ERRLOG("Unsufficient key buffer: %zu, %d needed",
			    *len, BN_num_bytes(bn));
		goto error;
	}
	rc = BN_bn2bin(bn, pub);
...
static EVP_PKEY *
nvme_auth_get_peerkey(const void *peerkey, size_t len, enum spdk_nvmf_auth_dhgroup dhgroup)
{
	EVP_PKEY_CTX *ctx = NULL;
	EVP_PKEY *result = NULL, *key = NULL;
	...
	ctx = EVP_PKEY_CTX_new_from_name(NULL, "DHX", NULL);
	if (ctx == NULL) {
		SPDK_ERRLOG("Failed to create dhkey peer ctx\n");
		goto error;
	}

	if (EVP_PKEY_fromdata_init(ctx) != 1) {
		SPDK_ERRLOG("Failed to initialie dhkey peer ctx\n");
		goto error;
	}
	...
	if (OSSL_PARAM_BLD_push_utf8_string(bld, "group",
		(char *)spdk_nvme_auth_get_dhgroup_name(dhgroup), 0) != 1) {
		goto error;
	}
			  ...
			  void *priv, size_t *privlen, enum spdk_nvmf_auth_dhgroup dhgroup)
{
	...
	if (ctx == NULL) {
		SPDK_ERRLOG("Failed to create dhsecret's ctx\n");
		return -ENOMEM;
	}
	if (EVP_PKEY_derive_init(ctx) != 1) {
		SPDK_ERRLOG("Failed to initialize dhsecret's ctx\n");
		rc = -EIO;
		goto out;
	}
	if (EVP_PKEY_CTX_set_dh_pad(ctx, 1) <= 0) {
		SPDK_ERRLOG("Failed to set dhsecret's padding\n");
		rc = -EIO;
		goto out;
	}
	...
	if (EVP_PKEY_derive(ctx, priv, privlen) != 1) {
...
nvme_auth_check_message(struct spdk_nvme_qpair *qpair, enum spdk_nvmf_auth_id auth_id)
{
	...
	const char *reasons[] = {
		...
		[SPDK_NVMF_AUTH_SC_CONCAT_MISMATCH] = "secure channel concatenation mismatch",
...
nvme_auth_send_negotiate(struct spdk_nvme_qpair *qpair)
{
	...
	for (i = 0; i < SPDK_COUNTOF(g_digests); ++i) {
		...
		AUTH_DEBUGLOG(qpair, "digest: %u (%s)\n", g_digests[i].id,
			      spdk_nvme_auth_get_digest_name(g_digests[i].id));
	...
	for (i = 0; i < SPDK_COUNTOF(g_dhgroups); ++i) {
		...
		AUTH_DEBUGLOG(qpair, "dhgroup: %u (%s)\n", g_dhgroups[i].id,
			      spdk_nvme_auth_get_dhgroup_name(g_dhgroups[i].id));
	...
	msg->t_id = auth->tid;
	msg->sc_c = SPDK_NVMF_AUTH_SC_NO_CONCAT;
	msg->napd = 1;
...
{
	struct spdk_nvmf_auth_challenge *challenge = qpair->poll_status->dma_data;
	struct nvme_auth *auth = &qpair->auth;
	uint8_t hl;
	int rc;

	rc = nvme_auth_check_message(qpair, SPDK_NVMF_AUTH_ID_CHALLENGE);
	if (rc != 0) {
		return rc;
	}
	...
	if (challenge->seqnum == 0) {
		AUTH_ERRLOG(qpair, "received challenge with sequence number=0\n");
		goto error;
	}
...
{
	struct spdk_nvmf_auth_success1 *msg = qpair->poll_status->dma_data;
	struct nvme_auth *auth = &qpair->auth;
	int rc;

	rc = nvme_auth_check_message(qpair, SPDK_NVMF_AUTH_ID_SUCCESS1);
	if (rc != 0) {
		return rc;
	}
