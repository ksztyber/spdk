...
uint8_t
spdk_nvme_auth_get_digest_length(int id)
{
	const struct nvme_auth_digest *digest = nvme_auth_get_digest(id);
...
nvme_auth_digest_allowed(struct spdk_nvme_qpair *qpair, uint8_t digest)
{
	...
	return ctrlr->opts.chap_digests & SPDK_BIT(digest);
}
...
nvme_auth_dhgroup_allowed(struct spdk_nvme_qpair *qpair, uint8_t dhgroup)
{
	...
	return ctrlr->opts.chap_dhgroups & SPDK_BIT(dhgroup);
}
...
int
spdk_nvme_auth_calculate(struct spdk_key *key, enum spdk_nvmf_auth_hash hash,
			 ...
			 const void *cval, void *rval)
{
	...
	hlen = spdk_nvme_auth_get_digest_length(hash);
...
nvme_auth_generate_dhkey(void *pub, size_t *len, enum spdk_nvmf_auth_dhgroup dhgroup)
{
	...
	if (EVP_PKEY_CTX_set_params(ctx, params) != 1) {
		SPDK_ERRLOG("Failed to set dhkey's dhgroup: %s\n",
			    spdk_nvme_auth_get_dhgroup_name(dhgroup));
		goto error;
	}
...
nvme_auth_derive_dhsecret(EVP_PKEY *key, const void *peer, size_t peerlen,
			  void *secret, size_t *seclen, enum spdk_nvmf_auth_dhgroup dhgroup)
{
	EVP_PKEY_CTX *ctx = NULL;
...
nvme_auth_check_challenge(struct spdk_nvme_qpair *qpair)
{
	...
	hl = spdk_nvme_auth_get_digest_length(challenge->hash_id);
	if (hl == 0) {
...
nvme_auth_send_reply(struct spdk_nvme_qpair *qpair)
{
	...
	rc = spdk_nvme_auth_calculate(ctrlr->opts.chap_key,
				      (enum spdk_nvmf_auth_hash)challenge->hash_id,
				      "HostHost", challenge->seqnum, auth->tid, false,
	...
	if (ctrlr->opts.chap_ctrlr_key != NULL) {
		...
		rc = spdk_nvme_auth_calculate(ctrlr->opts.chap_ctrlr_key,
					      (enum spdk_nvmf_auth_hash)challenge->hash_id,
					      "Controller", seqnum, auth->tid, false,
	...
	reply->t_id = auth->tid;
	reply->hl = hl;
	reply->cvalid = ctrlr->opts.chap_ctrlr_key != NULL;
	reply->dhvlen = publen;
	reply->seqnum = seqnum;
...
nvme_auth_check_success1(struct spdk_nvme_qpair *qpair)
{
	...
	if (ctrlr->opts.chap_ctrlr_key != NULL) {
		...
		hl = spdk_nvme_auth_get_digest_length(auth->hash);
		if (msg->hl != hl) {
...
{
	struct spdk_nvmf_auth_success2 *msg = qpair->poll_status->dma_data;
	struct nvme_auth *auth = &qpair->auth;

	memset(qpair->poll_status->dma_data, 0, NVME_AUTH_DATA_SIZE);
	msg->auth_type = SPDK_NVMF_AUTH_TYPE_DH_HMAC_CHAP;
	msg->auth_id = SPDK_NVMF_AUTH_ID_SUCCESS2;
	msg->t_id = auth->tid;
