...
class TraceProvider:
    ...
    def entries(self):
        """Generator returning consecutive trace entries"""
        raise NotImplementedError()
...
class NativeProvider(TraceProvider):
    """Trace provider based on SPDK's trace parser library"""
    def __init__(self, file):
    ...
    def _setup_binding(self, filename):
        self._lib = ct.CDLL('build/lib/libspdk_trace_parse.so')
    ...
    def _parse_defs(self):
        flags = ct.POINTER(CTraceFlags)()
        self._lib.spdk_trace_parser_get_flags(self._parser, ct.byref(flags))
    ...
    def entries(self):
        ...
        argconv = {TracepointArgument.TYPE_INT: lambda a: a.integer,
                   TracepointArgument.TYPE_PTR: lambda a: int(a.pointer),
