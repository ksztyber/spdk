...
if __name__ == "__main__":
    ...
    def bdev_nvme_set_options(args):
        rpc.bdev.bdev_nvme_set_options(args.client,
                                       ...
                                       chap_digests=args.chap_digests,
                                       chap_dhgroups=args.chap_dhgroups)
    ...
    p.add_argument('--chap-digests', help='Comma-separated list of allowed DH-HMAC-CHAP digests',
                   type=lambda d: d.split(','))
    p.add_argument('--chap-dhgroups', help='Comma-separated list of allowed DH-HMAC-CHAP DH groups',
                   type=lambda d: d.split(','))
    ...
    p.add_argument('--chap-key', help='DH-HMAC-CHAP key')
...
Format: 'user:u1 secret:s1 muser:mu1 msecret:ms1,user:u2 secret:s2 muser:mu2 msecret:ms2'""", required=False)
    ...
    *** Mutually exclusive with --disable-chap ***""", action='store_true')
    ...
    def keyring_add_key(args):
        rpc.keyring.keyring_add_key(**vars(args))

    p = subparsers.add_parser('keyring_add_key', help='Add a key to the keyring')
    p.add_argument('name', help='Name of the key to add')
    p.add_argument('module', help='Name of the keyring module')
    p.add_argument('--keyfile-path', help='Path of the file containing the key (only relevant for '
                   'the keyfile module)')
    p.set_defaults(func=keyring_add_key)

    def keyring_remove_key(args):
        rpc.keyring.keyring_remove_key(args.client, args.name)

    p = subparsers.add_parser('keyring_remove_key', help='Remove a key from the keyring')
    p.add_argument('name', help='Name of the key to remove')
    p.set_defaults(func=keyring_remove_key)
