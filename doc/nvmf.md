...
The SPDK NVMe-oF driver and NVMe-oF target both support in-band authentication via the DH-HMAC-CHAP
...
The authentication is enabled by specifying the keys associated with a given host/subsystem. Each
host is allowed to use a different key to connect to different subsystems and each subsystem might
use a different key for different hosts.  For instance, the following configures three hosts, two of
which can request bidirectional authentication:
...
SPDK NVMe-oF target will always select the strongest available hash/group depending on its
configuration and the capabilities of a peer.  Users can limit the allowed hash functions and/or
Diffie-Hellman groups via RPCs.  For example, the following limits the target (`nvmf_set_config`)
and the driver (`bdev_nvme_set_options`) to use sha384, sha512 and ffdhe6114, ffdhe8192:
