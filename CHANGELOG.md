...
Added the keyring library.  Its purpose is to provide secure access to cryptographic keys to other
libraries.  Prior to being used, keys need to be attached to the keyring using one of the modules.
Modules are responsible for retrieving keying material in a secure manner.  Currently, there are two
...
The NVMe driver now supports in-band authentication.  To enable it, users need to specify keys in
the `bdev_nvme_attach_controller` RPC.  Additionally, it's possible to limit the allowed digests and
Diffie-Hellman groups used during the authentication via `bdev_nvme_set_options`.
...
The NVMe-oF target now supports in-band authentication.  The target will request hosts to
authenticate if they're configured to use DH-HMAC-CHAP keys.  The keys can be set in the
`nvmf_subsystem_add_host` RPC.  Additionally, it's possible to limit the allowed digests and
Diffie-Hellman groups used during the authantication via `nvmf_set_config`.
